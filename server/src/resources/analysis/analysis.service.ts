...
class AnalysisService {
    ...
    private lookBestOption(dataObj: Analysis): Assignment | Error {
        try {
            const options = this.allOptions(dataObj);
            for (let i = 0; i < options.length; i++) {
                const option = options[i];
                let bool = true;
                for (let j = 0; j < option.result.length; j++) {
                    const point = option.result[j];
                    if (
                ...
                return { maxTotalDamage: 0, result: [] };
        } catch (error) {
